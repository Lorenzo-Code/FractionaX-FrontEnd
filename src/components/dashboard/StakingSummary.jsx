...
export default function StakingSummary() {
  ...
  const projectedValueUSD = (monthlyEstimate * 0.27).toFixed(2); // Assume 1 FXCT = $0.27
  ...
  return (
    <div className="bg-white rounded-xl shadow p-5 space-y-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-4">
        <p><strong>Staked:</strong> {stakedAmount.toLocaleString()} FXCT</p>
        <p><strong>Reward Rate:</strong> {(rewardRate * 100).toFixed(1)}% APY</p>
        <p><strong>Claimable:</strong> {claimableRewards} FXCT</p>
        <p><strong>Est. Monthly Rewards:</strong> ~{monthlyEstimate} FXCT (${projectedValueUSD})</p>
      </div>
