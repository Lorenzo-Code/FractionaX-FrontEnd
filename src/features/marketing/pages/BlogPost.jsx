...
const BlogPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const currentPost = publishedBlogs.find(blog => blog.slug === slug);
        
        setPost(currentPost);

        // Set related posts using the same data
        if (currentPost && publishedBlogs) {
          fetchRelatedPosts(currentPost, publishedBlogs);
        }
      } catch (err) {
  ...
  const fetchRelatedPosts = (currentPost, allBlogs) => {
    try {
