...
const UnifiedMap = ({ 
  ...
  onAmenitiesFound,
  clusterOptions = {},
  clusterThreshold = 10 // Properties count above which clustering is enabled
}) => {
  ...
  const shouldUseClustering = properties.length > clusterThreshold;
  ...
  // Create property markers for clustering
  const createPropertyMarkers = useCallback(() => {
    if (!mapRef.current || !window.google?.maps || !shouldUseClustering) return [];
    ...
    return markers;
  }, [properties, handleMarkerClick, shouldUseClustering]);

  // Setup clustering when map and properties change
  useEffect(() => {
    if (!mapRef.current || !isLoaded || !shouldUseClustering) return;
  ...
  }, [isLoaded, properties, createPropertyMarkers, finalClusterOptions, shouldUseClustering]);
  ...
  console.log('🗺️ UnifiedMap - Loading state:', { 
    isLoaded, 
    ...
    return (
      <div className="w-full bg-gray-100 border rounded p-4 flex items-center justify-center" style={{ height }}>
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading {shouldUseClustering ? 'Clustered' : 'Basic'} Map...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full border rounded overflow-hidden relative" style={{ height }}>
      ...
      >
        {/* Property markers - only render when NOT clustering */}
        {!shouldUseClustering && properties.map((property, index) => {
